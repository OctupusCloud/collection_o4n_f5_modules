...
$spec = @{
    options = @{
        ...
        type = @{ type = "str"; choices = "A", "AAAA", "CNAME", "DHCID", "NS", "PTR", "MX", "SRV", "TXT"; required = $true }
        value = @{ type = "list"; elements = "str"; default = @() ; aliases = @( 'values' ) }
        weight = @{ type = "int" }
        zone = @{ type = "str"; required = $true }
        computer_name = @{ type = "str" }
        # mailExchange = @{ type = "str" }
        preference = @{ type = "int"}
    }
    required_if = @(
        , @("type", "SRV", @("port", "priority", "weight"))
        , @("type", "MX", @("preference"))
        )
...
# $mailExchange = $module.Params.mailExchange
...
if ($null -ne $records) {
    ...
    foreach ($record in $records) {
        ...
        if (-Not $required_values.ContainsKey($record_value)) {
            ...
            elseif ($type -eq 'MX') {
                $record_preference_old = $record.RecordData.Preference.ToString()

                if ($record.TimeToLive -ne $ttl -or $preference -ne $record_preference_old) {
                    $new_record = $record.Clone()
                    $new_record.TimeToLive = $ttl
                    $new_record.RecordData.Preference = $preference
                    Set-DnsServerResourceRecord -ZoneName $zone -OldInputObject $record -NewInputObject $new_record -WhatIf:$module.CheckMode @extra_args

                    $changes.before += -join @(
                        "[$zone] $($record.HostName) $($record.TimeToLive.TotalSeconds) IN "
                        "$type $record_value $record_preference_old`n"
                    )
                    $changes.after += "[$zone] $($record.HostName) $($ttl.TotalSeconds) IN $type $record_value $preference`n"
                    $module.Result.changed = $true
                }
            }
            else {
...
if ($null -ne $values -and $values.Count -gt 0) {
    foreach ($value in $values) {
        ...
        try {
            ...
            elseif ($type -eq 'MX') {
                Add-DnsServerResourceRecord -Name $name -ZoneName $zone -TimeToLive $ttl @splat_args -Preference $preference -WhatIf:$module.CheckMode @extra_args
            }
            else {
